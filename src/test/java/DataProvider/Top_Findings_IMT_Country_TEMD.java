 ...
 *
 */
public class Top_Findings_IMT_Country_TEMD extends TemdGenGacdw{
	WebDriver driver ;
	...
	custPage.getServer();
	ScreenShot.captureScreenshot(driver, "Top findings IMT-country TEMD");
	...
	List<WebElement>CheckID=driver.findElements(By.xpath("html[1]/body[1]/div[1]/div[2]/div[1]/div[1]/div[2]/div[1]/div[3]/div[1]/table[1]/tbody[1]/tr/td[4]"));
					...
					{
			if(TemdViolprodOScan3.getField(20).equalsIgnoreCase(CheckMSG.get(i).getText())) {
	...
	for(String window1 : allWindowHandles1) {
		if(!window1.equals(currentWindowHandle1)) {
			...
			if(driver.getTitle().equalsIgnoreCase("ECM - Security Reporting on Demand | Health checking - Top findings customer message details")) {
				System.out.println(driver.getTitle());
				ScreenShot.captureScreenshot(driver, "Top findings customer message details");
				}
	...
	List<WebElement>CheckID=driver.findElements(By.xpath("html[1]/body[1]/div[1]/div[2]/div[1]/div[1]/div[2]/div[1]/div[3]/div[1]/table[1]/tbody[1]/tr/td[4]"));
					...
					for(int i=0;i<CheckMSG.size();i++)
					{
			if(TemdVioltransOScan3.getField(20).equalsIgnoreCase(CheckMSG.get(i).getText())) {
				System.out.println(CheckMSG.get(i).getText() +" ");
			System.out.println(i);
			...
			for(int j=0;j<CustLink.size();j++) {
				if(i==j) {
					System.out.println(j);
					ScreenShot.captureScreenshot(driver, "Top findings IMT-country");
					JavascriptExecutor Je3 = (JavascriptExecutor) driver;
					Je3.executeScript("arguments[0].click();", CustLink.get(j));
	...
	List<WebElement>CheckID=driver.findElements(By.xpath("html[1]/body[1]/div[1]/div[2]/div[1]/div[1]/div[2]/div[1]/div[3]/div[1]/table[1]/tbody[1]/tr/td[4]"));
					...
					for(int i=0;i<CheckMSG.size();i++)
					{
			if(TemdVioldeployOScan2.getField(19).equalsIgnoreCase(CheckMSG.get(i).getText())) {
				System.out.println(CheckMSG.get(i).getText() +" ");
			System.out.println(i);
			...
			for(int j=0;j<CustLink.size();j++) {
				if(i==j) {
					...
					for(String window : allWindowHandles) {
						if(!window.equals(currentWindowHandle)) {
							driver.switchTo().window(window);
							ScreenShot.captureScreenshot(driver, "Top findings customer message details TEMD");
							System.out.println(driver.getTitle());
	...
	//HC_topfindings_IMT_message_CANADA_CANADA_182_all_WWTS_all_ALL_ALL_190510
		...
		String FilePartialName1 = "HC_topfindings_IMT_message_"+ "MEA"+"_"+ IMT_COUNTRY.toUpperCase();
	...
	public void compareData(String fileName,int rowsize,String path,int columnSize)
	{
		...
		{ 
			...
			String webColum = Web_Column_HCTop_IMT_TEMD[a]; 
			int csvColum =	Integer.parseInt(CSV_Column_HCTop_IMT_TEMD[a]);
		...
		public void compareFindingsData(String fileName,int rowsize1,String path1,int columnSize)
		{
			...
			{ 
				System.out.println("**************FOR 1*******************");
				String webColum = Web_Column_HCTopMsg_IMT_TEMD[a]; 
				int csvColum =	Integer.parseInt(CSV_Column_HCTopMsg_IMT_TEMD[a]);
